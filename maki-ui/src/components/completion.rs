...
struct Active {
    ...
    plugin: Arc<str>,
    name: Arc<str>,
    /// What the user has typed after the trigger right now.
    query: String,
    /// The query the in-flight handler call was given. It trails `query`
...
impl Active {
    /// Ask the source for the current query. Replacing the receiver drops
    /// the previous one, so a late reply lands on a closed channel and never
    /// reaches `tick`.
    fn dispatch(&mut self, handle: &EventHandle) {
        self.pending = Some(handle.query_input_completer(
...
impl CompletionPopup {
    ...
    /// Labels currently on offer, for tests that care which reply landed.
    #[cfg(test)]
    fn items(&self) -> Vec<String> {
        self.active
            .as_ref()
            .map(|a| a.items.iter().map(|i| i.label.clone()).collect())
            .unwrap_or_default()
    }
    ...
    pub fn sync(&mut self, buffer: &TextBuffer) {
            ...
            && active.name == info.name
        {
            if active.query != query {
                // Items are cleared so Tab/Enter cannot pick an entry that
                // belongs to the text the user has already typed past.
                active.query = query;
                active.items.clear();
                active.selected = 0;
                if active.pending.is_none() {
                    active.dispatch(&self.handle);
                }
                // Otherwise a call is already out for an older query. Typing
                // faster than the source answers would otherwise start one
                // handler call per keystroke and leave every reply but the
                // last to be thrown away; `tick` dispatches the newest query
                // once the outstanding one lands.
            }
            return;
        }
    ...
    pub fn tick(&mut self) -> Dirty {
        ...
        match rx.try_recv() {
            Ok(Some(items)) => {
                active.pending = None;
                if active.dispatched == active.query {
                    active.items = items;
                    active.selected = 0;
                } else {
                    // The reply describes text the user has typed past. Drop
                    // it and ask for what is on screen now.
                    active.dispatch(&self.handle);
                }
                Dirty::YES
            }
        ...
        let lines: Vec<Line> = if active.items.is_empty() {
            ...
            active.items[window.clone()]
                ...
                .map(|(item, i)| {
                    ...
                    spans.extend(label_spans(&item.label, &item.indices, style, &t));
...
/// Split {label} so the chars the source matched on stand out, the way the
/// file picker highlights its own fuzzy matches. {indices} are 0-based char
/// offsets, ascending; an empty list gives one plain span.
fn label_spans<'a>(label: &str, indices: &[u32], base: Style, t: &theme::Theme) -> Vec<Span<'a>> {
    if indices.is_empty() {
...
mod tests {
    ...
    #[test]
    fn typing_faster_than_the_source_answers_keeps_one_call_in_flight() {
        let (mut popup, probe) = popup();
        let mut buf = buffer_at_end("@m");
        popup.sync(&buf);
        let (_, _, first) = probe
            .answer_completer_query(Some(vec![item("m.rs")]))
            .expect("the first keystroke queries");
        assert_eq!(first, "m");
        ...
        assert!(
            probe.answer_completer_query(Some(vec![])).is_none(),
            "three more keystrokes must not start three more calls"
        );

        // The outstanding reply describes "m", which the user has typed past,
        // so it is dropped and the current query goes out in its place.
        assert!(popup.tick().take());
        assert!(popup.items().is_empty(), "the stale reply must not show");
        let (_, _, latest) = probe
            .answer_completer_query(Some(vec![item("main.rs")]))
            .expect("the newest query follows the reply");
        assert_eq!(latest, "main");

        assert!(popup.tick().take());
        assert_eq!(popup.items(), ["main.rs"]);
    }

    #[test]
    fn match_indices_highlight_the_label() {
        let (mut popup, probe) = popup();
        popup.sync(&buffer_at_end("@mn"));
        probe.answer_completer_query(Some(vec![CompletionItem {
            label: "main.rs".into(),
            insert: "main.rs".into(),
            detail: None,
            indices: vec![0, 3],
        ...
        let t = theme::current();
        let spans = label_spans("main.rs", &[0, 3], t.item, &t);
    ...
    #[test]
    fn a_label_with_no_indices_renders_as_one_span() {
        let t = theme::current();
        let spans = label_spans("main.rs", &[], t.item, &t);
        assert_eq!(spans.len(), 1);
        assert_eq!(spans[0].content, "main.rs");
    }
