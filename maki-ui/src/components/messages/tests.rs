...
fn batch_entries(panel: &MessagesPanel) -> &[BatchToolEntry] {
    let ToolOutput::Batch { entries, .. } = panel.messages[0].output_ref().unwrap() else {
...
/// §11: a panel whose history is all deferred tool renders, each resolvable
/// through `Renders`. None are `Ready` until their segment is built.
...
fn resume_resolves_only_in_viewport_renders() {
    ...
    // One viewport page of margin each side: window ≈ 3·viewport_height rows.
    // Each tool stub is a few rows, so the resolved set is bounded by a small
    // multiple of the viewport, nowhere near the full history.
    assert!(
        ready <= 2 * 10 + 5,
...
fn first_frame_builds_only_newest_viewport_segments() {
    ...
    // The newest tool (last message) is at the auto-scrolled bottom, so its
    // render must be resolved on the first frame.
    assert!(matches!(
        panel.messages[VIEWPORT_TOOL_MSGS - 1].tool_output,
        Some(ToolOutputHandle::Ready(_))
    ));
    // The oldest tool is far above the viewport and stays deferred.
    assert!(matches!(
...
fn estimate_then_refine_height_on_scroll_in() {
    ...
    // Render once at the bottom viewport; out-of-viewport tools stay stubs.
    render(&mut panel, 80, 10);
    ...
    // Scroll all the way to the top: previously-stubbed segments scroll into
    // view and materialize, refining their stub estimates to true heights.
    panel.scroll_to_top();
    render(&mut panel, 80, 10);
    ...
    // The oldest tool is now in view and resolved.
    assert!(matches!(
        panel.messages[0].tool_output,
        Some(ToolOutputHandle::Ready(_))
    ));
