...
pub fn migrate_jsonl(sessions_dir: &Path, id: &str) -> Result<MigrateOutcome, StorageError> {
    ...
    // Step 1: exclusive lock on the sentinel `<id>.jsonl.migrate-lock`.
...
fn migrate_jsonl_locked(
    ...
    // Step 2: write temp folder; fsync every file + temp dir.
    ...
    let (marker, _out_count, _sub_msg_count) = write_folder(&scan, &temp_dir, id, "jsonl")?;

    // Step 3: verify against in-memory legacy records.
    verify_migration(&scan, &temp_dir, &marker)?;

    // Step 4: atomic-rename folder into place, then rename flat → .bak.
...
fn write_folder(
    ...
    id: &str,
    source: &str,
) -> Result<(MigrationMarker, usize, usize), StorageError> {
    ...
    let mut prev_id: Option<NodeRef> = None;
    for msg_raw in &scan.messages {
        ...
        prev_id = Some(NodeRef::Msg(node.id.clone()));
    ...
    // meta.json: latest meta + model + cwd from header
    let mut meta_record = build_meta_record(scan, &header);
    meta_record.migration = Some(MigrationMarker {
    ...
    let marker = meta_record.migration.clone().unwrap();
    Ok((marker, out_count, sub_msg_count))
}
...
/// Step 3: verify the migrated folder against the in-memory legacy records.
fn verify_migration(
    scan: &LegacyScan,
    dir: &Path,
...
/// Step 6: `.bak` deletion on a later successful, clean open with the writer
/// lock held. A `SessionReader`-only open never deletes anything.
pub fn maybe_delete_bak(
    sessions_dir: &Path,
    id: &str,
    marker: &MigrationMarker,
) -> Result<(), StorageError> {
...
/// Detect whether `sessions_dir/<id>.jsonl` is a legacy flat file.
pub fn has_flat_file(sessions_dir: &Path, id: &str) -> bool {
    sessions_dir.join(format!("{id}.jsonl")).exists()
}

pub fn flat_path(sessions_dir: &Path, id: &str) -> PathBuf {
