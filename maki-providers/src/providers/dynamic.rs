...
"#;
    ...
    /// `discover_in` only writes when `next != cache`, so a second run over an
    /// unchanged dir must rebuild the exact same cache, not just equal output.
    #[cfg(unix)]
    #[test]
    ...
    /// `next` is rebuilt from the directory each time, so a cache entry for a
    /// deleted script must not survive into the next run.
    #[cfg(unix)]
    #[test]
