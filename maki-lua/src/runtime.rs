                    ...
                    match msg {
                        ...
                        Request::RestoreToolAsync { item, event_tx } => {
                            ...
                            ex.spawn(async move {
                                ...
                                // Acquired before the timeout race starts, so
                                // the per-item deadline measures the item's own
                                // run, not time queued behind the whole batch.
                                let _gate_guard = g.acquire().await;
                                let id = item.tool_use_id.clone();
                                let theme_gen = item.theme_gen;
                                let tool = Arc::clone(&item.tool);
                                let res = futures_lite::future::race(
                                    restore_item(&lua, &plugins, item),
                                    async {
