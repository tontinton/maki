...
/// Read an item's optional `indices`: the label offsets to highlight.
/// Offsets past the end of the label are dropped rather than rejected, so a
/// source that highlights a string it later truncates degrades to no
/// highlight instead of failing the whole query.
fn indices_from_lua(item: &Table, label: &str, i: usize) -> mlua::Result<Vec<u32>> {
    let value = item.get::<mlua::Value>("indices")?;
    let mlua::Value::Table(list) = value else {
        return match value {
            mlua::Value::Nil => Ok(Vec::new()),
            other => Err(mlua::Error::runtime(format!(
                "items_from_lua: item at index {i} field 'indices' must be a list, got {}",
                other.type_name()
            ))),
        };
    };
    ...
    let mut out: Vec<u32> = list
        ...
        .map_err(|e| {
            mlua::Error::runtime(format!(
                "items_from_lua: item at index {i} field 'indices' must hold integers: {e}"
            ))
        })?;
    out.retain(|&idx| idx < len);
    out.sort_unstable();
    out.dedup();
    Ok(out)
}
...
pub(crate) fn items_from_lua(items: &Table) -> mlua::Result<Vec<CompletionItem>> {
    ...
    for (i, item) in items.sequence_values::<Table>().enumerate() {
        ...
        let indices = indices_from_lua(&item, &label, i)?;
...
///           is a list of `{ label (required), insert?, detail?, indices? }`.
///           `indices` are 0-based char offsets into `label` to highlight,
///           as returned by `maki.fn.matchfuzzypos`.
